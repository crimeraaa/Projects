#ifndef STRINGS_ASCII_H
#define STRINGS_ASCII_H
...
#define ASCII_SET_LIMB_BITS  (sizeof(ASCII_SET_LIMB_TYPE) * CHAR_BIT)
...
ascii_set_get_limb_ptr(ascii_Set *set, char c, ASCII_SET_LIMB_TYPE *mask)
{
    ...
    curr   = cast(ASCII_SET_LIMB_TYPE)c;
    ...
    *it++ = '\\';
    ...
    default:
        if (ascii_is_control(c)) {
            it += convert_base(cast(uint)c, /*base=*/8, it);
        } else {
